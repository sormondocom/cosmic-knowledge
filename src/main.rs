...
fn main() {
    ...
    if args.contains(&"--export-all".to_string()) {
        println!(
            "{}",
            "🎵 Exporting all Solfeggio frequencies…"
                .bold()
                .bright_magenta()
        );
    ...
    let audio_system: Option<AudioSystem> = if enable_audio {
        match initialize_audio() {
            ...
            Err(_) => {
                println!(
                    "{}",
                    "⚠️  Audio unavailable — continuing in silent mode".yellow()
                );
                None
            }
        }
        ...
        match show_main_menu() {
            ...
            MainMode::Frequencies => {
                if audio_system.is_some() {
                    ...
                    println!(
                        "{}",
                        "⚠️  Audio unavailable — run without --silent to enable exports.".yellow()
                    );
                }
            }
