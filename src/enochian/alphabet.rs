...
pub fn show_enochian_table() {
    ...
    println!(
        "{}",
        "║          📜 THE ENOCHIAN ALPHABET — John Dee / Edward Kelley (1582)      ║"
            .bold()
            .bright_yellow()
    );
    println!(
        "{}",
        "║    Source: Sloane MS 3189 (Loagaeth) · Laycock, Complete Enochian Dict.  ║".dimmed()
    );
    println!(
        "{}",
        "╠══════════════════════════════════════════════════════════════════════════╣"
    ...
    println!(
        "{}",
        "║  Substitutions (no Enochian glyph):                                      ║"
            .italic()
            .dimmed()
    );
    println!(
    ...
    println!(
        "{}",
        "║  Note: Dee's originals contain NO explicit gematria table.               ║"
            .italic()
            .dimmed()
    );
    println!(
        "{}",
        "║  The Ordinal system (pos. 1-21) is most defensible; the GD values        ║"
            .italic()
            .dimmed()
    );
    println!(
        "{}",
        "║  are a 19th-century Golden Dawn retrofitting (Mathers/Regardie).         ║"
            .italic()
            .dimmed()
    );
    println!(
...
mod tests {
    ...
    #[test]
    fn substitute_j_to_i() {
        assert_eq!(enochian_substitute('J'), 'I');
    }

    #[test]
    fn substitute_k_to_c() {
        assert_eq!(enochian_substitute('K'), 'C');
    }

    #[test]
    fn substitute_w_to_v() {
        assert_eq!(enochian_substitute('W'), 'V');
    }
