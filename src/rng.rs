...
fn configure() -> Config {
    ...
    println!(
        "  {}",
        "─ Set draw interval ──────────────────────────────────────".dimmed()
    );
    print!(
        "  {} ",
        "▸ Seconds between each draw (1–60) [default 3]:"
            .bold()
            .cyan()
    );
    io::stdout().flush().unwrap_or(());
    ...
    let delay_secs = dbuf.trim().parse::<f64>().unwrap_or(3.0).clamp(1.0, 60.0);
...
fn identify_user() -> Option<SessionContext> {
    ...
    if name.is_empty() {
        println!(
            "{}",
            "  Running anonymously — session will not be recorded.".dimmed()
        );
        return None;
    }
    ...
    let conn = match open_db() {
        ...
        Err(e) => {
            println!(
                "{}",
                format!(
                    "  ⚠️  History database unavailable ({}). Running anonymously.",
                    e
                )
                .yellow()
            );
    ...
    match get_or_create_user(&conn, &name) {
        Ok((user, true)) => {
            println!(
                "{}",
                format!("  ✨ Welcome, {}! Your psi history begins now.", user.name).bright_cyan()
            );
...
pub fn run_rng_session() {
    ...
    println!(
        "{}",
        "║  Can focused intention influence a hardware random number  ║".dimmed()
    );
    println!(
        "{}",
        "║  generator?  Think of a number — watch for your match.    ║".dimmed()
    );
    println!(
        "{}",
        "╚════════════════════════════════════════════════════════════╝".bright_magenta()
    ...
    println!(
        "  {}",
        "─ How to participate ─────────────────────────────────────".dimmed()
    );
    println!(
        "  1. Choose a number between {} and {} — hold it clearly in your mind.",
        cfg.min.to_string().bright_white(),
        cfg.max.to_string().bright_white()
    );
    println!(
        "  2. Numbers will draw every {} second(s).",
        format!("{:.1}", cfg.delay_secs).bright_white()
    );
    println!(
        "  3. When the number shown matches yours: type {} and press Enter.",
        "Y".bold().bright_green()
    );
    println!(
        "  4. To end without a confirmed match:   type {} and press Enter.",
        "Q".bold().bright_red()
    );
    println!();
    print!(
        "  {} ",
        "Ready? Press Enter to begin…".italic().bright_yellow()
    );
    io::stdout().flush().unwrap_or(());
    ...
    thread::spawn(move || loop {
        ...
        if tx.send(line.trim().to_lowercase().to_string()).is_err() {
            break;
        }
    });
    ...
    println!(
        "  {}",
        "─ Session running ────────────────────────────────────────".dimmed()
    );
    println!(
        "  {}",
        "[Y = match · Q = stop · or just watch and wait]".dimmed()
    );
    println!();
    ...
    if let Some(ref ctx) = ctx {
        ...
        if let Err(e) = record_session(
            ...
            println!(
                "{}",
                format!("  ⚠️  Could not save session: {}", e).yellow()
            );
        }
...
fn show_results(cfg: &Config, outcome: &Outcome) {
    ...
    match outcome {
        Outcome::Match { on_draw } => {
            ...
            println!(
                ...
                format!(
                    "{:.1}% chance of matching by draw {} by luck alone",
                    p * 100.0,
                    on_draw
                )
                .dimmed(),
        ...
        Outcome::Stopped { total } => {
            ...
            println!(
                ...
                format!(
                    "{:.1}% chance of 0 hits in {} draws by luck alone",
                    p * 100.0,
                    total
                )
                .dimmed(),
    ...
    println!(
        "{}",
        "  ║  Note: a single trial is not statistically conclusive.   ║".dimmed()
    );
    println!(
        "{}",
        "  ║  Repeat sessions accumulate evidence over time.          ║".dimmed()
    );
    println!(
        "{}",
        "  ╚══════════════════════════════════════════════════════════╝".bright_magenta()
...
pub fn print_cumulative_stats(name: &str, stats: &CumulativeStats) {
    ...
    println!(
        ...
        format!(
            "{}/{} sessions",
            stats.beat_chance_count, stats.total_sessions
        )
        .bright_cyan(),
    );
    ...
    if stats.total_sessions < 10 {
        ...
        println!(
            "{}",
            "  ║  Trends emerge after ~10 sessions — keep experimenting. ║"
                .italic()
                .dimmed()
        );
    }
