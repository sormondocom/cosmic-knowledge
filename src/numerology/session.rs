...
pub fn run_numerology_session(audio_system: &Option<AudioSystem>) {
    ...
    println!(
        "{}",
        "║  Hebrew · Pythagorean · Chaldean · Greek Isopsephy       ║".dimmed()
    );
    println!(
        "{}",
        "║  Agrippan · Simple Ordinal · Reverse Ordinal · Abjad     ║".dimmed()
    );
    println!(
        "{}",
        "║  Vedic (Anka Vidya) · Enochian Ordinal · Enochian G.D.  ║".dimmed()
    );
    println!(
        "{}",
        "╚══════════════════════════════════════════════════════════╝".bright_white()
    ...
    if audio_system.is_some() {
        println!(
            "{}",
            "🎵 Frequencies will attune to each word's root number."
                .italic()
                .bright_blue()
        );
    }
    ...
    loop {
        ...
        match input.to_lowercase().as_str() {
            "systems" => {
                ...
                println!(
                    "{}",
                    "  (restart session from main menu to change)".dimmed()
                );
                continue;
            }
            "help" => {
        ...
        for (system, (total, root)) in &results {
            ...
            if *system == "Vedic" {
                ...
                println!(
                    "      {}",
                    format!("  Bīja: {} | Colors: {}", vr.bija, vr.color).dimmed()
                );
            }
            if !is_enochian {
        ...
        if active_systems.contains(&"Enochian Ordinal") {
            if let Some((_, (_, er))) = all_results.iter().find(|(s, _)| *s == "Enochian Ordinal") {
                ...
                println!(
                    "  {}",
                    format!("   {}", enochian_angelic_message(*er))
                        .italic()
                        .bright_yellow()
                );
            }
        ...
        if let (Some(ref sys), Some(root)) = (audio_system, first_root) {
            ...
            println!(
                "\n  {}",
                format!("🎵 Frequency attuned to: {} Hz ({})", freq, fname)
                    .italic()
                    .bright_magenta()
            );
        }
        ...
        match prompt_export_format() {
            ExportChoice::Text => {
                export_text(&stem, &build_numerology_report(&word, &active_systems))
            }
            ExportChoice::Html => {
                export_html(&stem, &build_numerology_html(&word, &active_systems))
            }
            ExportChoice::Pdf => {
                export_pdf(&stem, &build_numerology_report(&word, &active_systems))
            }
            ExportChoice::Skip => {}
        }
        println!();
...
fn choose_numerology_systems() -> Vec<&'static str> {
    ...
    println!(
        "    {}  {}",
        "A.".cyan(),
        "All systems (default)".bright_white()
    );
    println!();
    print!(
        "{}",
        "  ▸ Enter numbers separated by spaces (e.g. 1 3 4) or A for all: ".cyan()
    );
    io::stdout().flush().unwrap_or(());
    ...
    if selected.is_empty() {
        println!(
            "{}",
            "  No valid selection — defaulting to all systems.".yellow()
        );
        ALL.to_vec()
    } else {
        println!(
            "  {}",
            format!("✓ {} system(s) selected.", selected.len()).bright_green()
        );
        selected
    }
}
