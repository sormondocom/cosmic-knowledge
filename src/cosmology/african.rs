...
fn build_ifa_odu_report(n: u8, odu: &IfaOdu) -> String {
    use chrono::Utc;
    let timestamp = Utc::now().format("%Y-%m-%d  %H:%M:%S UTC").to_string();
    let mut out = String::new();
    out.push_str("YORUBA IFA ODU REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    out.push_str(&format!("Index  : {}\n", n));
    out.push_str(&format!("Name   : {}\n", odu.name));
    out.push_str(&format!("Orisha : {}\n", odu.orisha));
    ...
    out.push_str("Source: Abimbola, W. Ifa: An Exposition (1976, OUP Nigeria)\n");
    out
}

fn build_akan_day_report(d: u8, day: &AkanDay) -> String {
    use chrono::Utc;
    let timestamp = Utc::now().format("%Y-%m-%d  %H:%M:%S UTC").to_string();
    let mut out = String::new();
    out.push_str("AKAN DAY NAME REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    out.push_str(&format!("Day number  : {} (0=Sunday)\n", d));
    out.push_str(&format!("English day : {}\n", day.day_en));
    out.push_str(&format!("Akan name   : {}\n", day.day_akan));
    out.push_str(&format!("Male name   : {}\n", day.male_name));
    out.push_str(&format!("Female name : {}\n", day.female_name));
    out.push_str(&format!("Guardian    : {}\n", day.orisa));
    out.push_str(&"─".repeat(60));
    out.push('\n');
...
fn build_kemetic_report(n: u32, meaning: &str) -> String {
    use chrono::Utc;
    let timestamp = Utc::now().format("%Y-%m-%d  %H:%M:%S UTC").to_string();
    let mut out = String::new();
    out.push_str("KEMETIC / ANCIENT EGYPTIAN NUMBER REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    ...
    out.push_str(&"─".repeat(60));
    out.push('\n');
    out.push_str(&format!("Significance:\n  {}\n", meaning));
    out.push('\n');
    out.push_str(&"═".repeat(60));
    out.push_str("\nGenerated by Celestial Numerology Analyzer — African Cosmology\n");
    out.push_str("Sources: Morenz (1973); Wilkinson (2003); Faulkner (1972)\n");
    out
}
