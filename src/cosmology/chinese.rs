...
pub fn nine_star_info(star: u32) -> NineStarInfo {
    match star {
        ...
        _ => NineStarInfo {
            ...
            trigram: "—",
            symbol: "?",
            qualities: "Star out of range (1–9 only)",
        },
    }
}
...
pub fn chinese_lucky_meaning(n: u32) -> Option<ChineseLuckyInfo> {
    match n {
        ...
        168 => Some(ChineseLuckyInfo {
            number: 168,
            mandarin: "yī-liù-bā (一六八)",
            cantonese: "yat-luk-baat",
            luck: "Extremely Lucky",
        ...
        518 => Some(ChineseLuckyInfo {
            number: 518,
            mandarin: "wǔ-yī-bā (五一八)",
            cantonese: "ng-yat-baat",
            luck: "Lucky",
            meaning: "'I want to prosper' (我要發 wǒ yào fā / 我要發 ǹg yiu faat). \
                      Popular in business contexts",
        }),
        666 => Some(ChineseLuckyInfo {
            number: 666,
            mandarin: "liù-liù-liù (六六六)",
            cantonese: "luk-luk-luk",
            luck: "Very Lucky",
        ...
        888 => Some(ChineseLuckyInfo {
            number: 888,
            mandarin: "bā-bā-bā (八八八)",
            cantonese: "baat-baat-baat",
            luck: "Extremely Lucky",
...
pub fn run_chinese_session() {
    loop {
        ...
        println!(
            "{}",
            "║   1.  🌟  Nine Star Ki — Natal Star from Birth Year      ║".bright_white()
        );
        println!(
            "{}",
            "║          Ba Gua trigram · Wu Xing element · qualities    ║".dimmed()
        );
        println!(
            "{}",
            "║                                                          ║".bright_yellow()
        ...
        println!(
            "{}",
            "║   2.  🍀  Lucky / Unlucky Number Lookup                  ║".bright_white()
        );
        println!(
            "{}",
            "║          Mandarin & Cantonese homophony meanings         ║".dimmed()
        );
        println!(
            "{}",
            "║                                                          ║".bright_yellow()
        ...
        println!(
            "{}",
            "║   3.  🌊  Wu Xing — Five Elements for a Lo Shu Number    ║".bright_white()
        );
        println!(
            "{}",
            "║          Element, direction, season, qualities           ║".dimmed()
        );
        println!(
            "{}",
            "║                                                          ║".bright_yellow()
...
fn chinese_nine_star_session() {
    ...
    let adjusted_year = if early_buf.trim().eq_ignore_ascii_case("y") {
        year - 1
    } else {
        year
    };
    ...
    println!(
        "  {}",
        format!(
            "║  🌟  Nine Star Ki for birth year {}{}             ║",
            year,
            if adjusted_year != year {
                " (adj.)"
            } else {
                "         "
            }
    ...
    let stem = format!("nine_star_ki_{}", adjusted_year);
    match prompt_export_format() {
        ExportChoice::Text => {
            let report = build_nine_star_report(year, adjusted_year, &info, &wx);
            export_text(&stem, &report);
        }
        ExportChoice::Html | ExportChoice::Pdf => {
            // TODO: HTML builder for Chinese cosmology
            let report = build_nine_star_report(year, adjusted_year, &info, &wx);
            export_text(&stem, &report);
            println!(
                "{}",
...
fn chinese_lucky_session() {
    ...
    println!(
        "{}",
        "  Enter a number to check its Chinese lucky/unlucky meaning.".dimmed()
    );
    println!(
        "{}",
        "  (1–9 and compound numbers 18, 28, 68, 88, 99, 168, 518, 666, 888)".dimmed()
    );
    ...
    loop {
        ...
        match chinese_lucky_meaning(n) {
            None => println!(
                "{}",
                format!(
                    "  ℹ️  {} has no specific documented homophony meaning in this database.",
                    n
                )
                .dimmed()
            ),
            Some(info) => {
                ...
                println!(
                    ...
                    format!(
                        "Mandarin: {}  |  Cantonese: {}",
                        info.mandarin, info.cantonese
                    )
                    .dimmed()
                );
                println!("  {}", info.meaning.bright_white());

                let stem = format!("chinese_lucky_{}", n);
                match prompt_export_format() {
                    ExportChoice::Text => {
                        let report = build_lucky_report(n, &info);
                        export_text(&stem, &report);
                    }
                    ExportChoice::Html | ExportChoice::Pdf => {
                        // TODO: HTML builder for Chinese cosmology
...
fn chinese_wu_xing_session() {
    ...
    println!(
        "{}",
        "  Enter a Lo Shu position (1–9) to see its Wu Xing element.".dimmed()
    );
    println!("{}", "  (empty line → back)".dimmed());
        ...
        match wu_xing(n) {
            ...
            Some(wx) => {
                ...
                println!(
                    ...
                    wx.symbol,
                    wx.element_zh.bold(),
                    wx.element_en.bright_green().bold()
                );
                println!(
                    "  Direction: {}  |  Season: {}",
                    wx.direction.cyan(),
                ...
                let stem = format!("wu_xing_{}", n);
                match prompt_export_format() {
                    ExportChoice::Text => {
                        let report = build_wu_xing_report(n, &wx);
                        export_text(&stem, &report);
                    }
                    ExportChoice::Html | ExportChoice::Pdf => {
                        // TODO: HTML builder for Chinese cosmology
...
fn build_nine_star_report(
    ...
    out.push_str("NINE STAR KI REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    out.push_str(&format!("Birth year    : {}", year));
    if adjusted_year != year {
        out.push_str(" (adjusted for pre-Risshun birth)");
    }
    out.push('\n');
    out.push_str(&format!(
        "Natal Star    : {}  —  {}\n",
    ...
    out.push_str(&format!(
        "Wu Xing       : {} ({})\n",
        wx.element_en, wx.element_zh
    ));
    out.push_str(&format!(
        "Direction     : {}  |  Season: {}\n",
        wx.direction, wx.season
    ));
    out.push_str(&format!("Wu Xing notes : {}\n", wx.qualities));
    out.push('\n');
    out.push_str(&"═".repeat(60));
    out.push_str("\nGenerated by Celestial Numerology Analyzer — Chinese Cosmology\n");
    out
}

fn build_lucky_report(n: u32, info: &ChineseLuckyInfo) -> String {
    use chrono::Utc;
    let timestamp = Utc::now().format("%Y-%m-%d  %H:%M:%S UTC").to_string();
    let mut out = String::new();
    out.push_str("CHINESE LUCKY NUMBER REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    out.push_str(&format!("Number     : {}\n", n));
    out.push_str(&format!("Luck       : {}\n", info.luck));
    out.push_str(&format!("Mandarin   : {}\n", info.mandarin));
    out.push_str(&format!("Cantonese  : {}\n", info.cantonese));
    out.push_str(&"─".repeat(60));
    out.push('\n');
...
fn build_wu_xing_report(n: u32, wx: &WuXingInfo) -> String {
    use chrono::Utc;
    let timestamp = Utc::now().format("%Y-%m-%d  %H:%M:%S UTC").to_string();
    let mut out = String::new();
    out.push_str("WU XING (FIVE ELEMENTS) REPORT\n");
    out.push_str(&format!("Generated: {}\n", timestamp));
    out.push_str(&"═".repeat(60));
    out.push('\n');
    out.push_str(&format!("Lo Shu number : {}\n", n));
    out.push_str(&format!(
        "Element       : {} ({})\n",
        wx.element_en, wx.element_zh
    ));
