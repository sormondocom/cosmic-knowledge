...
fn build_nine_star_report(year: i32, adjusted_year: i32, info: &NineStarInfo, wx: &WuXingInfo) -> String {
    use chrono::Utc;
    ...
    out.push('\n');
    out.push_str(&format!("Natal Star    : {}  —  {}\n", info.star, info.name));
    out.push_str(&format!("Trigram       : {} {}\n", info.symbol, info.trigram));
    ...
    out.push_str(&"─".repeat(60));
    out.push('\n');
    out.push_str(&format!("Wu Xing       : {} ({})\n", wx.element_en, wx.element_zh));
    out.push_str(&format!("Direction     : {}  |  Season: {}\n", wx.direction, wx.season));
